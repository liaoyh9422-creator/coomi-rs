...
fn render_header(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("模型  ", Style::default().fg(theme::MUTED)),
            Span::styled(
...
fn render_brand(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    lines.push(Line::from(vec![
        Span::styled("清晰构建。", Style::default().fg(theme::MUTED)),
        Span::styled("稳步推进。", Style::default().fg(theme::CORAL)),
    ]));
    let mode = format!("{} 个工具  ·  {}", 7, app.policy.label());
...
fn render_recent_sessions(frame: &mut Frame<'_>, area: Rect, app: &TuiState, divider: bool) {
    ...
    let mut lines = vec![Line::from(vec![
        Span::styled("最近", Style::default().fg(theme::SKY).bold()),
        Span::styled(
            format!("  {} 个会话", app.sessions.len()),
            Style::default().fg(theme::MUTED),
        ),
    ])];
    lines.push(Line::from(""));
    ...
    for entry in &app.timeline {
        ...
        match entry {
            TimelineEntry::User(content) => {
                lines.push(Line::from(vec![
                    Span::styled("你  ", Style::default().fg(theme::MINT).bold()),
                    Span::styled("用户", Style::default().fg(theme::MUTED)),
                ]));
            ...
            TimelineEntry::Assistant(content) => {
                lines.push(Line::from(vec![
                    ...
                    Span::styled("助手", Style::default().fg(theme::MUTED)),
                ]));
                lines.extend(markdown_lines_with_width(content, width));
            }
            TimelineEntry::Reasoning(content) => {
                lines.push(Line::from(vec![
                    Span::styled("思考中  ", Style::default().fg(theme::MUTED).bold()),
                    Span::styled(
            ...
            TimelineEntry::SideUser(content) => {
                lines.push(Line::from(vec![
                    Span::styled("旁路你  ", Style::default().fg(theme::SKY).bold()),
                    Span::styled("临时", Style::default().fg(theme::MUTED)),
                ]));
                lines.extend(plain_content_lines(content, theme::TEXT));
            }
            TimelineEntry::SideAssistant(content) => {
                lines.push(Line::from(vec![
                    Span::styled("旁路 COOMI  ", Style::default().fg(theme::ORANGE).bold()),
                    Span::styled("只读", Style::default().fg(theme::MUTED)),
                ]));
                lines.extend(markdown_lines_with_width(content, width));
            }
            TimelineEntry::Tool {
                ...
                let (mark, mark_style, status) = match state {
                    ...
                    ToolState::Complete { success: true, .. } => {
                        ("✓".into(), Style::default().fg(theme::SUCCESS), "完成")
                    }
                    ToolState::Complete { success: false, .. } => {
                        ("×".into(), Style::default().fg(theme::ERROR), "失败")
                    }
                };
                lines.push(Line::from(vec![
...
fn render_footer(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    let hint = if area.width >= 86 {
        Line::from(vec![
            ...
            Span::styled(" 设置  ", Style::default().fg(theme::MUTED)),
            Span::styled("Alt+H", theme::key()),
            Span::styled(" 帮助", Style::default().fg(theme::MUTED)),
        ])
    } else {
        Line::from(vec![
            Span::styled(" Ctrl+K", theme::key()),
            Span::styled(" 菜单  ", Style::default().fg(theme::MUTED)),
            Span::styled("Alt+H", theme::key()),
            Span::styled(" 帮助", Style::default().fg(theme::MUTED)),
        ])
    };
...
fn render_models(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    render_picker(
        ...
        Picker {
            title: " 模型 ",
            subtitle: &format!("当前  {current}"),
            query: &query,
            items,
            selected: overlay.selected,
            footer: "Enter 切换  ·  输入以筛选  ·  Esc 关闭",
        },
    );
}
...
fn render_history(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    render_picker(
        ...
        Picker {
            title: " 会话历史 ",
            subtitle: &format!("此工作区保存了 {} 个会话", app.sessions.len()),
            query: &query,
            items,
            selected: overlay.selected,
            footer: "Enter 恢复  ·  D 删除  ·  输入以筛选  ·  Esc 关闭",
        },
    );
}
...
fn render_picker(frame: &mut Frame<'_>, area: Rect, picker: Picker<'_>) {
    ...
    // At very short terminal heights, keep the title and selectable content visible.
    // The subtitle, search row, and footer are secondary and can be restored when space allows.
    if inner.height < 7 {
        let selected = picker.selected.min(picker.items.len().saturating_sub(1));
        let (start, end) = visible_range(picker.items.len(), selected, inner.height as usize);
        let visible = picker
            .items
            .into_iter()
            .skip(start)
    ...
    let search = if picker.query.is_empty() {
        Line::from(Span::styled("  搜索...", Style::default().fg(theme::MUTED)))
    } else {
        Line::from(vec![
            Span::styled("  / ", Style::default().fg(theme::MINT)),
...
fn render_catalog(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    let tabs = Line::from(vec![
        ...
        Span::raw("  "),
        catalog_tab(" 技能 ", overlay.catalog_tab == CatalogTab::Skills),
    ]);
...
fn render_help(frame: &mut Frame<'_>, area: Rect) {
    ...
    frame.render_widget(Clear, popup);
    let block = modal_block(" 快捷键 ");
    let inner = block.inner(popup);
    frame.render_widget(block, popup);
    let lines = vec![
        help_line("Ctrl+K", "打开命令面板"),
        help_line("Ctrl+R", "打开会话历史"),
        help_line("Alt+M", "切换已配置模型"),
        help_line("Alt+S", "打开设置"),
        help_line("Alt+H", "打开快捷键帮助"),
        help_line("Alt+L", "创建或控制 Loop"),
        help_line("Shift+Tab", "切换访问策略"),
        help_line("Enter", "发送；运行中则加入队列"),
        help_line("Shift+Enter", "插入新行"),
        help_line("PageUp/PageDown", "滚动对话"),
        help_line("Esc", "关闭、取消，或连按两次退出"),
        help_line("Ctrl+C", "取消当前轮次、清空输入或退出"),
        Line::from(""),
        Line::from(Span::styled(
            "斜杠命令: /status /compact /model /history /loop /plan /memory /mcp /skills /settings /new /clear /quit",
            Style::default().fg(theme::MUTED),
        )),
    ];
    frame.render_widget(
...
fn render_mcp_form(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::styled("  本地配置", Style::default().fg(theme::MUTED)),
        ])),
        rows[0],
    );
    for (index, (parameter, editor)) in form.fields.iter().enumerate() {
        ...
        frame.render_widget(
            Paragraph::new(if value.is_empty() {
                Span::styled("必填", Style::default().fg(theme::MUTED))
            } else {
...
fn render_settings(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    let block = Block::default()
        ...
        .title(Line::from(vec![
            Span::styled(" COOMI 设置 ", theme::title()),
            Span::styled(
                " 供应商 · MCP · 技能 · 运行时 ",
                Style::default().fg(theme::MUTED),
            ),
    ...
    let settings_rows = Layout::default()
        ...
        .split(inner);
    let tabs = [
        (SettingsTab::Providers, "1 供应商"),
        (SettingsTab::Mcp, "2 MCP"),
        (SettingsTab::Skills, "3 技能"),
        (SettingsTab::Runtime, "4 运行时"),
    ];
    frame.render_widget(
        ...
        List::new(items)
            .block(
                Block::default()
                    .borders(Borders::RIGHT)
                    .border_style(Style::default().fg(theme::BORDER))
                    .title(Span::styled(" 供应商 ", theme::title())),
            )
            .highlight_style(theme::selected())
            .highlight_symbol(" "),
        columns[0],
        ...
        && let Some(provider) = settings.document.providers.get(id)
    {
        ...
        let details = vec![
            setting_line("显示名称", &provider.display),
            setting_line("协议类型", &provider.provider_type),
            setting_line(
                "工具协议",
                provider.tool_protocol.as_deref().unwrap_or("default"),
            ),
            setting_line("Base URL", &provider.base_url),
            setting_line("模型", &provider.model),
            setting_line("快速模型", provider.fast_model.as_deref().unwrap_or("-")),
            ...
            setting_line(
                ...
                &provider
                    .context_window
                    .map(compact_tokens)
                    .unwrap_or_else(|| "128.0K 默认".into()),
            ),
            setting_line(
                "有效窗口",
                &provider
                    .effective_context_window_percent
                    .map_or_else(|| "95% 默认".into(), |value| format!("{value}%")),
            ),
            setting_line(
                "压缩阈值",
                &provider
            ...
            setting_line("视觉", &provider.supports_vision.to_string()),
            setting_line("原生搜索", &provider.supports_web_search.to_string()),
            Line::from(""),
            Line::from(Span::styled(
...
fn render_service_settings(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    if settings.tab == SettingsTab::Runtime {
        ...
        let lines = vec![
            setting_line("版本", env!("CARGO_PKG_VERSION")),
            setting_line("更新", &app.update_status),
            setting_line("供应商", &provider),
            setting_line("访问策略", app.policy.label()),
            setting_line("Coomi home", &app.home.display().to_string()),
            setting_line("工作区", &app.cwd.display().to_string()),
            Line::from(""),
            Line::from(vec![
    ...
    match settings.tab {
        SettingsTab::Mcp => {
            ...
            let items = mcp_items
                ...
                .collect::<Vec<_>>();
            render_settings_list(frame, columns[0], " 推荐 MCP ", items, settings.selected);
            if let Some(item) = mcp_items.get(settings.selected) {
                    ...
                    status.map_or_else(
                        ...
                        |status| {
                            status
                                .error
                                .clone()
                                .unwrap_or_else(|| format!("已连接，{} 个工具", status.tools_count))
                        },
                    )
                } else {
                    "可安装".to_owned()
                ...
                let target = item
                    ...
                    .unwrap_or("-");
                let installed = item.configured.as_ref().map_or("否", |_| "是");
                let description = item
                    .entry
                    .as_ref()
                    .map(|entry| entry.description.as_str())
                    .unwrap_or("自定义本地 MCP 配置");
                frame.render_widget(
                    Paragraph::new(vec![
                        setting_line("名称", name),
                        setting_line("已安装", installed),
                        setting_line("传输", transport),
                        setting_line("目标", target),
                        setting_line("状态", &state),
                        Line::from(""),
                        Line::from(Span::styled(
            ...
            render_settings_footer(
                frame,
                rows[1],
                "Enter 安装/配置   Space 启用/禁用   R 测试   D 移除",
                app.settings_busy,
            );
        }
        SettingsTab::Skills => {
            ...
            let items = skill_items
                ...
                .collect::<Vec<_>>();
            render_settings_list(frame, columns[0], " 推荐技能 ", items, settings.selected);
            if let Some(item) = skill_items.get(settings.selected) {
                ...
                let name = item
                    ...
                    .map(|entry| entry.name.as_str())
                    .unwrap_or_else(|| item.id());
                let installed = item.installed.as_ref().map_or("否", |_| "是");
                let source = item
                    .installed
                    .as_ref()
                ...
                frame.render_widget(
                    Paragraph::new(vec![
                        setting_line("名称", name),
                        setting_line("已安装", installed),
                        setting_line("来源", source),
                        setting_line("提交", &commit),
                        setting_line("路径", &path),
                        Line::from(""),
                        Line::from(Span::styled(
                            description.to_owned(),
                            Style::default().fg(theme::MUTED),
            ...
            render_settings_footer(
                frame,
                rows[1],
                "Enter 安装   Space 启用/禁用   U 更新   D 卸载",
                app.settings_busy,
            );
        }
...
fn render_provider_form(
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::raw(" 取消   "),
            Span::styled(
                format!("字段 {}/{}   ", form.selected + 1, form.fields.len()),
                Style::default().fg(theme::MUTED),
            ),
            Span::styled(
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(" 仅本次批准    ", Style::default().fg(theme::MUTED)),
                Span::styled("N / Esc", theme::key()),
                Span::styled(" 拒绝", Style::default().fg(theme::MUTED)),
            ]),
        ])
        .wrap(Wrap { trim: true }),
        inner,
...
fn render_user_input(frame: &mut Frame<'_>, area: Rect, app: &TuiState) {
    ...
    items.push(ListItem::new(Line::from(vec![
        Span::styled("其他", Style::default().bold()),
        Span::styled("  输入自定义答案", Style::default().fg(theme::MUTED)),
    ])));
    ...
    let custom = pending.other_editor.as_ref().map_or_else(
        ...
        |editor| {
            Line::from(vec![
                Span::styled("自定义  ", Style::default().fg(theme::ORANGE).bold()),
...
fn render_delete_confirmation(
    ...
    let (message, blocked) = match target {
        super::DeleteTarget::Session(id) if *id == app.session.id => {
            ("不能删除当前活动会话。".to_owned(), true)
        }
        super::DeleteTarget::Session(_) => {
            ("删除这个已保存会话？此操作不可撤销。".to_owned(), false)
        }
        super::DeleteTarget::Provider(id) => {
            (format!("删除供应商 `{id}`？此操作不可撤销。"), false)
        }
        super::DeleteTarget::Mcp(name) => (format!("从配置中移除 MCP `{name}`？"), false),
        super::DeleteTarget::Skill(name) => {
            (format!("卸载技能 `{name}` 并删除其本地文件？"), false)
        }
    };
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(if blocked {
                vec![Span::styled("Esc", theme::key()), Span::raw(" 关闭")]
            } else {
                vec![
...
mod tests {
    ...
    fn logical_row(buffer: &Buffer, y: u16, width: u16) -> String {
        let mut row = String::new();
        let mut x = 0;
        while x < width {
            let symbol = buffer[(x, y)].symbol();
            row.push_str(symbol);
            let cell_width = UnicodeWidthStr::width(symbol).max(1);
            x = x.saturating_add(u16::try_from(cell_width).unwrap_or(1));
        }
        row
    }

    fn cell_x_for_text(buffer: &Buffer, y: u16, width: u16, needle: &str) -> Option<u16> {
        let mut row = String::new();
        let mut positions = Vec::new();
        let mut x = 0;
        while x < width {
            let symbol = buffer[(x, y)].symbol();
            positions.push((row.len(), x));
            row.push_str(symbol);
            let cell_width = UnicodeWidthStr::width(symbol).max(1);
            x = x.saturating_add(u16::try_from(cell_width).unwrap_or(1));
        }
        ...
        positions
            .into_iter()
            .find_map(|(offset, x)| (offset == start).then_some(x))
    }
    ...
    fn rendered_rows(state: &TuiState, width: u16, height: u16) -> Vec<String> {
        ...
        (0..height).map(|y| logical_row(buffer, y, width)).collect()
    }
    ...
    fn welcome_renders_at_desktop_and_narrow_sizes() {
        ...
        for (width, height) in [(110, 34), (52, 20)] {
            ...
            let mut terminal = Terminal::new(backend).expect("test terminal");
            terminal.draw(|frame| draw(frame, &state)).expect("draw");
            let rendered = rendered_rows(&state, width, height).join("\n");
            assert!(rendered.contains("COOMI"));
            assert!(rendered.contains("消息"));
        }
    }
    ...
    fn timeline_and_history_overlay_render_without_overlap_panics() {
        ...
        let mut terminal = Terminal::new(backend).expect("test terminal");
        terminal.draw(|frame| draw(frame, &state)).expect("draw");
        let rendered = rendered_rows(&state, 48, 18).join("\n");
        assert!(rendered.contains("会话历史"));
    }
    ...
    fn approval_and_questions_render_above_the_composer() {
        ...
        let approval_y = rows
            .iter()
            .position(|row| row.contains("需要确认"))
            .expect("approval row");
        let composer_y = rows
            .iter()
            .position(|row| row.contains("消息"))
            .expect("composer row");
        ...
        let question_y = rows
            .iter()
            .position(|row| row.contains("问题 1/1"))
            .expect("question row");
        let composer_y = rows
            .iter()
            .position(|row| row.contains("消息"))
    ...
    fn header_and_footer_use_semantic_colors_without_ready_status() {
        ...
        let footer = logical_row(buffer, 33, 110);
        assert!(footer.contains("workspace-write"));
        assert!(footer.contains("上下文 82% 已用"));
        assert!(!footer.contains("Ready"));
        let policy_x = cell_x_for_text(buffer, 33, 110, "workspace-write").expect("policy");
        let context_x = cell_x_for_text(buffer, 33, 110, "上下文 82% 已用").expect("context");
    ...
    fn provider_form_pages_advanced_fields_on_narrow_terminals() {
        ...
        assert!(rendered.contains("编辑供应商"));
        assert!(rendered.contains("并行工具"));
    }
