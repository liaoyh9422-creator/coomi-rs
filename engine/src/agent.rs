...
use crate::upgrades::context_guard::context_pressure_message;
use crate::upgrades::model_recovery::{
    EMPTY_RESPONSE_MAX_RETRIES, REASONING_ONLY_MAX_RETRIES, RecoveryStreamObserver,
    ResponseRecovery, classify_response, is_context_window_error, retry_instruction,
};
use std::fmt;
use std::sync::Arc;
...
impl Agent {
    ...
    async fn run_turn_message(
        ...
        for round in 1..=self.max_tool_rounds {
            ...
            let mut reasoning_only_retries = 0u8;
            let response = loop {
                ...
                let stream_observer = RecoveryStreamObserver::new(observer);
                ...
                let recovery = classify_response(&response, stream_observer.saw_reasoning());
                let (attempt, max_retries) = match recovery {
                    ResponseRecovery::Continue => break response,
                    ResponseRecovery::RetryEmpty if empty_retries < EMPTY_RESPONSE_MAX_RETRIES => {
                        empty_retries += 1;
                        (empty_retries, EMPTY_RESPONSE_MAX_RETRIES)
                    }
                    ResponseRecovery::RetryReasoningOnly
                        if reasoning_only_retries < REASONING_ONLY_MAX_RETRIES =>
                    {
                        reasoning_only_retries += 1;
                        (reasoning_only_retries, REASONING_ONLY_MAX_RETRIES)
                    }
                    _ => break response,
                };
                if let Some(instruction) = retry_instruction(recovery, attempt, max_retries) {
                    session
                        .messages
                        .push(ChatMessage::internal_user(instruction));
                }
                continue;
            };
            ...
            observer.on_event(&AgentEvent::ContextUpdated(status.clone()));
            let pressure_message = context_pressure_message(&status);

            if response.tool_calls.is_empty() {
                if let Some(message) = pressure_message {
                    session.messages.push(ChatMessage::internal_user(message));
                }
            ...
            crate::upgrades::execute_tool_calls(
                session,
                response.tool_calls,
                tools,
                approval,
...
// Model response recovery helpers live in upgrades::model_recovery.
...
mod tests {
    ...
    struct EmptyThenDoneProvider {
        calls: Mutex<usize>,
    }

    #[async_trait]
    impl ModelProvider for EmptyThenDoneProvider {
        fn provider_id(&self) -> &str {
            "mock"
        }
    ...
    #[tokio::test]
    async fn retries_empty_model_response_with_internal_instruction() {
        let mut session = Session::new("mock", "empty-then-done", PathBuf::from("."));
        let provider = EmptyThenDoneProvider {
            calls: Mutex::new(0),
        };
        let output = Agent::new("test")
            .run_turn(
            ...
            .await
            .expect("empty response recovery");
        assert_eq!(output, "done");
        assert_eq!(*provider.calls.lock().expect("lock calls"), 2);
    }
